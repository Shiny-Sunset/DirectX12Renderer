...
DirectX 12 の学習用リポジトリです。ウィンドウ生成から順に積み上げ、Blender で自作した 3D モデルの描画までを実装しました。現在は、このレンダラーを土台に 3D アクションゲームを作ることを目標にしています。
...
操作できるのは手前の 1 体です。残りの 2 体は、同じモデルのデータを共有したまま別々のアニメーションを再生しています。
